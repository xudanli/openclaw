...
export async function sanitizeSessionMessagesImages(
  ...
  options?: {
    ...
    sanitizeThoughtSignatures?: ThoughtSignatureSanitizeOptions;
  },
  ...
  for (const msg of base) {
    ...
    if (role === "assistant") {
      ...
      if (Array.isArray(content)) {
        ...
        const strippedContent = stripThoughtSignatures(
          content,
          options?.sanitizeThoughtSignatures,
        );
    ...
    if (role === "assistant") {
      ...
      const newContent = assistantMsg.content.map((block) => {
        ...
        if (
          ...
          const signature =
            blockRecord.thought_signature ?? blockRecord.thoughtSignature;
