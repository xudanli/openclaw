...
function isGeminiThoughtSignatureModel(params: {
  modelApi?: string | null;
  provider?: string;
  modelId?: string;
}): boolean {
  if (isGoogleModelApi(params.modelApi)) return true;
  const provider = (params.provider ?? "").toLowerCase();
  if (provider !== "openrouter" && provider !== "opencode") return false;
...
async function sanitizeSessionHistory(params: {
  ...
  const geminiLike = isGeminiThoughtSignatureModel(params);
  const sanitizedImages = await sanitizeSessionMessagesImages(
    ...
    "session:history",
    {
      ...
      sanitizeThoughtSignatures: geminiLike
